...
export default class DinoGame extends GameRunner {
  constructor(width, height) {
    ...
    this.defaultSettings = {
      bgSpeed: 9, // ppf
      birdSpeed: 9, // ppf
      birdSpawnRate: 245, // fpa
      birdWingsRate: 15, // fpa
      cactiSpawnRate: 45, // fpa //75 let me hit level 1000
  ...
  getFunFact() {
    const funFacts = [
      'Steam whistles were most often used in factories, but also in other similar places such as railway locomotives, tractions engines, and steamships',
      'An array of steam whistles arranged to play music is referred to as a calliope',
      "There are different types of steam whistles such as Organ Whistles, Gongs, Plain Whistles, and Chime Whistles, most likely Llyod Miller's whistles.",
      'The earliest use of steam whistles was as boiler low-water alarms in the 18th century',
      'Steam whistle serves the purpose of generating acoustic signals',
      'Stokes was a division of the Exide Coportation of Trenton, originally known as The Electric Storage Battery Company. It is now most commonly knows as Exide Technologies.',
      'The whistle consists of these parts: the Steam orifice or aperture, and the valve',
      'Wally was the last whistle to ever be heard in Trenton',
      'Bill Miller was employed by Stokes Molded products ESB a division of exide corporation of Trenton as the company’s maintenance supervisor from 1950 to the next to the last employee in 1988',
      'Wally is currently located in the Trenton Free Public Libraries Trentoniana Exhibit',
    ]
    const funFactNum = Math.floor(Math.random() * 7)
    const funFact = funFacts[funFactNum]
    return funFact
  }
  ...
  async preload() {
    ...
    const [spriteImage] = await Promise.all([
      loadImage('./assets/whistlesprites.png'),
  ...
  endGame() {
    ...
    this.paintSprite(
      ...
      this.height / 2.3 - iconSprite.h / 4 + padding
    )

    const factOutput = 'Fun Fact: ' + this.state.funFact

    this.paintText(factOutput, this.width / 3, this.height / 1.5 - padding, {
      font: 'PressStart2P',
      size: '12px',
      align: 'center',
      baseline: 'bottom',
  ...
  updateScore() {
    ...
    if (state.score.value > this.highScore) {
      this.highScore = state.score.value
    }
  }
  ...
  drawScore() {
    ...
    let highDrawValue = this.highScore
      ...
      if (score.blinks > 7) {
        ...
        if (score.blinks % 2 === 0) {
          ...
          if (this.highScore % 100 === 0) {
            highDrawValue = Math.floor(highDrawValue / 100) * 100
          }
        } else {
          shouldDraw = false
        }
